...
void	add_sphere(t_object *world, t_object_p* o_ptr)
{
	...
	oadd(&world, object(SP, sphere(o_ptr->sphere->coordinates, o_ptr->sphere->diameter / 2.0), vdivide(o_ptr->sphere->colors, RGB_MAX)));
}
...
void	add_cylinder(t_object *world, t_object_p* o_ptr)
{
	...
	oadd(&world, object(CY, cylinder(o_ptr->cylinder->coordinates, o_ptr->cylinder->normal_vector, o_ptr->cylinder->diameter / 2.0, o_ptr->cylinder->height), vdivide(o_ptr->cylinder->colors, RGB_MAX)));
}
...
void	add_cone(t_object *world, t_object_p* o_ptr)
{
	...
	oadd(&world, object(CO, cone(o_ptr->cone->coordinates, o_ptr->cone->normal_vector, o_ptr->cone->diameter / 2.0, o_ptr->cone->height), vdivide(o_ptr->cone->colors, RGB_MAX)));
}