...
t_color3	get_ambient(t_ambient_p *a_ptr)
{
	...
	return (vmult(vdivide(a_ptr->colors, RGB_MAX), a_ptr->ratio));
}
...
t_object	*get_light(t_light_p *l_ptr)
{
	...
	return (object(LIGHT_POINT, light_point(l_ptr->coordinates, vdivide(l_ptr->colors, RGB_MAX), l_ptr->brightness), color3(0, 0, 0)));
}
